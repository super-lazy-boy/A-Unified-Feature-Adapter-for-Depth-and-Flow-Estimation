...
from model.utils.utils import induced_flow, check_cycle_consistency
...
class LayeredFlow(data.Dataset):
    def __init__(self, root='data/public_layeredflow_benchmark', downsample=4, split='val'):
...
class SpringFlowDataset(FlowDataset):
    ...
    """
    def __init__(self, aug_params=None, root='data/spring', split='train', subsample_groundtruth=True):
...
def fetch_dataloader(args, rank=0, world_size=1, use_ddp=False):
    ...
    if args.dataset == 'chairs':
        ...
        clean_dataset = FlyingThings3D(aug_params, dstype='frames_cleanpass', root=args.paths['things'])
        final_dataset = FlyingThings3D(aug_params, dstype='frames_finalpass', root=args.paths['things'])
        train_dataset = clean_dataset + final_dataset

    elif args.dataset == 'sintel':
        aug_params = {'crop_size': args.image_size, 'min_scale': args.scale - 0.2, 'max_scale': args.scale + 0.6, 'do_flip': True}
        sintel_clean = MpiSintel(aug_params, split='training', dstype='clean', root=args.paths['sintel'])
        sintel_final = MpiSintel(aug_params, split='training', dstype='final', root=args.paths['sintel'])
        ...
        train_dataset = SpringFlowDataset(aug_params, subsample_groundtruth=True, root=args.paths['spring'])
        ...
        aug_params = {'crop_size': args.image_size, 'min_scale': args.scale - 0.2, 'max_scale': args.scale + 0.6, 'do_flip': True}
        things = FlyingThings3D(aug_params, dstype='frames_cleanpass', root=args.paths['things'])
        sintel_clean = MpiSintel(aug_params, split='training', dstype='clean', root=args.paths['sintel'])
        sintel_final = MpiSintel(aug_params, split='training', dstype='final', root=args.paths['sintel'])
        kitti = KITTI({'crop_size': args.image_size, 'min_scale': args.scale - 0.3, 'max_scale': args.scale + 0.5, 'do_flip': True}, root=args.paths['kitti'])
        hd1k = HD1K({'crop_size': args.image_size, 'min_scale': args.scale - 0.5, 'max_scale': args.scale + 0.2, 'do_flip': True}, root=args.paths['hd1k'])
